...
const headers = [
	...
	{ title: t('id'), key: '@id' },
	{ title: t('event.title'), key: 'title', sortable: false },
	...
	<v-container fluid>
		...
		>
			...
			<template #item.venue="{ item }">
				<router-link v-if="router.hasRoute('VenueShow')" :to="{ name: 'VenueShow', params: { id: item.raw.venue } }">
					{{ item.raw.venue }}
				</router-link>

				<!-- TODO v-text here and 2 above and 2 below -->
				<p v-else>{{ item.raw.venue }}</p>
			</template>

			<template #item.users="{ item }">
				<template v-if="router.hasRoute('UserShow')">
					<router-link v-for="user in item.raw.users" :key="user" :to="{ name: 'UserShow', params: { id: user } }">
						{{ user }}
						<br />
					</router-link>
				</template>
				...
				<template v-else>
					<p v-for="user in item.raw.users" :key="user">{{ user }}</p>
				</template>
			</template>
		</v-data-table-server>
	</v-container>
